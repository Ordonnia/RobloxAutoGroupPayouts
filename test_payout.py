"""
Diagnostic probe + live payout test.

    python test_payout.py --user-id 1899503253 --amount 1 --probe-only
    python test_payout.py --user-id 1899503253 --amount 1
    python test_payout.py --user-id 1899503253 --amount 1 --runs 3
"""
...
def run_payout(rp, user_id: int, amount: int, run_num: int) -> bool:
    hr(f"Payout #{run_num}")
    t0 = time.time()
...
def main() -> None:
    ...
    if not args.skip_checks:
        ...
        check_group(ROBLOSECURITY, GROUP_ID, auth["id"])
    ...
    for i in range(1, args.runs + 1):
        ok = run_payout(rp, args.user_id, args.amount, i)
        results.append(ok)
        if not ok:
